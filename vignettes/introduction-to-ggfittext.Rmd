...
defined area. It's possible to achieve this by manually fiddling with the font
size, but this is both tedious and un-reproducible. ggfittext provides a geom
called `geom_fit_text()` that automatically resizes text to fit inside a box.
It works like this:
...
`coord_polar()` to the plot. This feature is experimental and currently only in
the development version; any bug reports are very welcome.
...
- **`outside`** is `FALSE` by default for `geom_fit_text()`. If `TRUE`, text
  ...
  bars in a bar plot, so it is `TRUE` by default for `geom_bar_text()`.
...
- **`fullheight`** is automatically set depending on place, but can be
  ...
  text and descenders, but not any descenders. If TRUE, it extends from the top
